...
| PATCH | `/queue/{id}/status` | staff, worker, admin | body: `{status}`, one of `queue`/`waiting`/`washing`/`ready`/`no_show` (never `canceled` — that's `/cancel`; 400 `invalid_status` otherwise). Forward one step at a time, plus `no_show` from `queue`/`waiting`, plus `queue` to restore a `no_show`/`canceled` booking (409 `slot_unavailable` if its slot was taken, 409 `active_booking_exists` if the customer has another active booking). 409 `invalid_status_transition` on anything else, including any move from `ready`. Scoped to the caller's own washing point — 404 `queue_not_found` otherwise. |
...
| GET | `/washing-points/{id}/queue/day` | staff, worker, admin | cabinet "Очередь" day view: every booking that calendar day (`?date=YYYY-MM-DD`, default today), **all statuses** (incl. `ready`/`canceled`/`no_show`), with full staff-only detail — `ticket`, `car_name`, `plate`, `client_name`, `client_phone`, `service_name`, `price_option_name`, `price_cents`, `source`. Own point only (404 `washing_point_not_found`). |
| POST | `/washing-points/{id}/queue/manual` | staff, worker, admin | "Добавить вручную": book a walk-in. Body `{service_id, price_option_id, box_number, scheduled_start_at, client_phone (the only required client field), car_name?, plate?, client_name?}`. `client_phone` may omit the country code — normalized to E.164, bare local numbers get `+992`. Finds-or-creates the client as a `customer` by phone; if `car_name`/`plate` is given their car is upserted (matched by plate, else name), otherwise the booking has no car (`car_id` = zero UUID). Start may be now/past. Status `queue`, `source: manual`. 409 `slot_unavailable` / `active_booking_exists` / `phone_not_customer`; also `outside_operating_hours`, `box_closed`, `invalid_price_option`. Returns the day-item shape. |
