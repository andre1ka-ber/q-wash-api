...
| POST | `/washing-points/{id}/queue/manual` | staff, worker, admin | "Добавить вручную": book a walk-in. Body `{service_id, price_option_id, box_number, scheduled_start_at, car_name, plate?, client_phone (required, E.164), client_name?}`. Finds-or-creates the client as a `customer` by phone and creates a car row for them. Start may be now/past. Status `queue`, `source: manual`. 409 `slot_unavailable` / `active_booking_exists` / `phone_not_customer`; also `outside_operating_hours`, `box_closed`, `invalid_price_option`. Returns the day-item shape. |
